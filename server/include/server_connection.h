...
namespace pine
{
	...
	public:
		...
		async_task send_ack_async(snowflake id);

		void send_ack(snowflake id);

		/// @brief Close the connection and remove it from the server.
		/// @return An asynchronous task completed when the connection has been closed.
		async_task close_async();
	...
	private:
		...
		async_operation<bool> establish_connection_async();
		...
		async_operation<bool> validate_connection_async();
