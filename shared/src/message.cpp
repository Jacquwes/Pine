...
namespace pine::socket_messages
{
	...
	void message_header::parse(std::vector<uint8_t> const& buffer)
	{
		...
		type_of_message = static_cast<message_type>(buffer[0]);


		uint64_t id;
		std::memcpy(&id, &buffer[sizeof(type_of_message) + sizeof(body_size)], sizeof(id));
	...
	std::vector<uint8_t> message_header::serialize() const
	{
		...
		buffer.push_back(static_cast<uint8_t>(type_of_message));
		buffer.resize(size);

		// copy body size
		std::memcpy(&buffer[sizeof(type_of_message)], &body_size, sizeof(body_size));

		// copy message id
		uint64_t id = message_id;

		std::memcpy(&buffer[sizeof(type_of_message) + sizeof(body_size)], &id, sizeof(id));
